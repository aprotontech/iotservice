                ...
                <Form ref="formData" :model="formData" :rules="ruleValidate" :label-width="80" style="width: 500px">
                    <FormItem label="权限标示" prop="permission">
                        <Input v-model="formData.permission" placeholder="example：ota_view"
                            :disabled="disablePermission"></Input>
                    </FormItem>
    ...
    methods: {
            ...
            this.$refs[name].validate((valid) => {
                if (valid) {
                    ...
                    if (this.permissionId <= 0) {
                        ...
                        this.$axios.post(this.$API.PERMISSION_PERM_ADD, params)
                            .then(response => {
                        ...
                        this.$axios.post(this.$API.PERMISSION_PERM_EDIT, params)
                            .then(response => {
                                console.log(response);
                                if (response.data && response.data.rc == '0') {
                                    this.$Message.success('成功!');
        ...
        queryData() {
            ...
            if (id > 0) {
                ...
                this.$axios.post(this.$API.PERMISSION_PERM_SHOW, params)
                    .then(response => {
                        console.log(response);
                        if (response.data && response.data.rc == '0') {
