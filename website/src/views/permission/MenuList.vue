...
<div>
    ...
    <Table :loading=loading :data="tableData1" :columns="tableColumns1" stripe>
        <template slot-scope="{ row, index }" slot="action">
            <Button type="primary" size="small" style="margin-right: 5px" :to="{name:'menuedit', params:{ id:row.id }}" >编辑</Button>
...
export default {
        ...
        methods: {
            fetchData () {
                ...
                this.$axios
                     .post(this.$API.PERMISSION_MENU_LIST, params)
                     .then(response => {
            ...
            onDel(id) {
                if(confirm('你确定要删除吗？')) {
                    let params = {
                        ...
                        this.$axios
                             .post(this.$API.PERMISSION_MENU_DEL, params)
                             .then(response => {
                                console.log(response);
                                if(response.data && response.data.rc == '0') {
