                    ...
                    <Form ref="formSearch" :model="formSearch" inline>
                        <FormItem>
                            <Button type="info" icon="md-add"
                                :to="{ name: 'apiadd', query: { p: formSearch.permission, system: formSearch.system } }">Add</Button>
                        </FormItem>
                        <FormItem>
                            <Input type="text" v-model="formSearch.name" placeholder="搜索接口地址" style="width: 300px" />
                        </FormItem>
                        <FormItem>
                            <Input type="text" v-model="formSearch.permission" placeholder="权限标示"
                                style="width: 300px" />
                        </FormItem>
                        <FormItem>
                    ...
                    <Table :loading=loading :data="tableData1" :columns="tableColumns1" stripe>
                        <template slot-scope="{ row, index }" slot="action">
                            <Button type="primary" size="small" style="margin-right: 5px"
                                :to="{ name: 'apiedit', params: { id: row.id } }">编辑</Button>
...
export default {
    ...
    methods: {
        fetchData() {
            ...
            this.$axios.post('/admin/api/list', params)
                .then(response => {
        ...
        onDel(id) {
            if (confirm('你确定要删除吗？')) {
                ...
                this.$axios.post('/admin/api/del', params)
                    .then(response => {
                        console.log(response);
                        if (response.data && response.data.rc == '0') {
