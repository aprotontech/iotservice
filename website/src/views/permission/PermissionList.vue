            ...
            <Card>
                ...
                <div class="form-search">
                    ...
                    <Form ref="formSearch" :model="formSearch" inline>
                        <FormItem>
                            <Button type="info" icon="md-add" to='permission'>Add</Button>
                        </FormItem>
                        <FormItem>
                            <Input type="text" v-model="formSearch.name" placeholder="搜索" style="width: 300px">
                            </Input>
                        </FormItem>
                        <FormItem>
                    ...
                    <Table :loading=loading :data="tableData1" :columns="tableColumns1" stripe>
                        <template slot-scope="{ row, index }" slot="action">
                            <Button type="primary" size="small" style="margin-right: 5px"
                                :to="{ name: 'permissionedit', params: { id: row.id } }">编辑</Button>
                            <!--Button type="primary" size="small" style="margin-right: 5px" :to="{name:'apis', query:{ p:row.permission, system:row.system }}">API</Button-->
...
export default {
    ...
    methods: {
        fetchData() {
            ...
            let params = {
                permission: this.formSearch.name,
                page: this.page,
                sys: this.formSearch.system,
                size: this.pagesize
            }
            this.$axios.post(this.$API.PERMISSION_PERM_LIST, params)
                .then(response => {
        ...
        onDel(id) {
            if (confirm('你确定要删除吗？')) {
                ...
                this.$axios.post(this.$API.PERMISSION_PERM_DEL, params)
                    .then(response => {
                        console.log(response);
                        if (response.data && response.data.rc == '0') {
